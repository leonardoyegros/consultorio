...
function livesearch(options){
	...
	selector.keyup(function(e){
		...
		if(e.keyCode != 38 && e.keyCode != 40 && e.keyCode != 13){
			...
			for (var i = 0; i < data.length; i++) {
				if (data[i]['Expense']['name'].match(re)) {
					tr =  '<div index="'+i+'" class="option row" onclick="'+options['callback']+'('+i+');">';
		...
		if($('#livesearch .option').length == 0){
			...
			$('#livesearch').append('<div class="option row"><a href="#" style="display:inherit; width:100%;" data-toggle="modal" data-target="#create'+options['model']+'"><?php echo __("Add New", true)?></a></div>');
		}
