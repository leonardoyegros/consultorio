 ...
 */
	function label($fieldName = null, $text = null, $options = array()) {
		...
		return sprintf(
			...
			$labelFor,
			$this->_parseAttributes($options), __($text, true)
		);
	}
 ...
 */
	function input($fieldName, $options = array()) {
		...
		// echo $fieldset; print_r($options); die();

		// $this->log("============================", "inputs");
		// $this->log($fieldName, "inputs");
		// $this->log(print_r($options,true), "inputs");
		...
		if (!isset($options['type']) && $fieldName != 'id' && empty($options['options']) && empty($options['data-live-search'])&&$options['class']!='selectpicker show-tick') {
			$options['type'] = 'text';
			// $this->log("text", "inputs");
		}
		...
		';
		$options['before'] = sprintf($options['before'],isset($options['md-cols'])?$options['md-cols']:3);  //DEFINICION DE CANTIDAD DE COLUMNAS

		$after = !empty($options['suffix']) ?  '<span class="input-group-addon" id="basic-addon2">'.$options['suffix'].'</span>' : '';
		...
		switch ($type) {
			...
			case 'checkbox':
				...
				$this->log(print_r($input,true), 'inputs');
				$format = $format ? $format : array('before', 'label','input', 'between', 'after', 'error');
				$this->log(print_r($format,true), 'inputs');
			break;
			case 'radio':
			...
			case 'textarea':
				...
				// print_r($options); 
				// die("ASDASDAS");
				$input =$this->textarea($fieldName, $options + array('cols' => '30', 'rows' => '5'));
			default:
				$input = $this->{$type}($fieldName, $options);
 ...
 */
	function submit($caption = null, $options = array()) {
		...
		$options['before'] = '<div class="col-md-12"><div class="form-group">';
